...
export default async function createOne(req, res) {
  ...
  const countDocumentsOfIPAddress = await Calculation.countDocuments({
    ipAddress,
  });
  if (countDocumentsOfIPAddress === 20) {
    const getLastRecord = await Calculation.find({ ipAddress })
      .sort({ createdAt: -1 })
      .limit(1)
      .lean()
      .exec();
  ...
  const allRecords = await Calculation.find({ ipAddress })
    .sort({ created: -1 })
    .lean()
    .exec();
  res
    .status(201)
    .send({ data: allRecords, currentRecord: doc, count: allRecords.length });
}
